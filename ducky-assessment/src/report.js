...
// ─── File Edit Aggregation ────────────────────────────────────────────────────

function aggregateFileEvents(fileEvents) {
  const byFile = {};
  const times = {};
  let bursts = 0;
  let totalAdded = 0;
  ...
  return { byFile, times, bursts, totalAdded };
}

// ─── Process / Network Aggregation ───────────────────────────────────────────
...
function aggregateSamples(samples) {
  ...
  return { aiProcs, aiHosts };
}

// ─── Report Builder ───────────────────────────────────────────────────────────

/**
 * Aggregate raw events and session metadata into the final report, write it to
 * `ducky-report.json`, and return the report object.
 * @param {{ projectDir: string, events: string, report: string }} p - Paths object from state.paths().
 * @param {{ startedAt?: string, endedAt?: string, gitStart?: object, gitEnd?: object, aiArtifactsStart?: object[], aiArtifactsEnd?: object[] }} session
...
export function buildReport(p, session) {
  ...
  const { aiProcs, aiHosts } = aggregateSamples(samples);
  const { byFile, times, bursts, totalAdded } = aggregateFileEvents(fileEvents);
